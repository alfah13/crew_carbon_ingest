...
def run_ops_plant_b():
    ...
    for name, fpath in files.items():
        try:
            ...
            # Clean columns
            df.columns = ['_'.join(str(x) for x in col).strip()
            ...
            date_col = [col for col in df.columns if 'DATE' in col][0] if any(
                'DATE' in col for col in df.columns) else None
            ...
            if date_col:
                ...
                dropped_summaries = 0
                logger.warning(
                    f"  Could not find DATE column to filter TOTAL/MAX/MIN/AVG")
    ...
    combined_df.rename(columns={"EFFLUENT DATA_7_FIN_EFF_FLOW_Unnamed: 7_level_5_MGD": "actual_eff_flow_mgd",
    ...
    return combined_df[["actual_eff_flow_mgd", "actual_inf_flow_mgd", "date", "plant_id", 'source_file']]

if __name__ == "__main__":
    from sqlalchemy import create_engine, inspect
    from src.models.schemas import WastewaterPlants, WasteWaterPlantOps, CrewCarbonLabReadings
    import os
    import json
    from src.utils.logging_config import setup_logger
    import pandas as pd
    ...
    print(f"✓ Successfully wrote all data to ops_plant_data")
